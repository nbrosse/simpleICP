...
class SimpleICPOptimization:
    ...
    def __init__(
        ...
        rbtp_initial_values: Tuple[float],
        rbtp_observed_values: Tuple[float],
        rbtp_observation_weights: Tuple[float],
    ) -> None:
        """[summary]
        ...
        Args:
            ...
            rbtp_observed_values (Tuple[float]): See docstring of SimpleICP.run.
            rbtp_observation_weights (Tuple[float]): See docstring of SimpleICP.run.
        """
        self._cp = corr_pts

        self._rbtp = RigidBodyParameters()
        self._rbtp.set_parameter_attributes_from_list(
            "initial_value", rbtp_initial_values
        )
        self._rbtp.set_parameter_attributes_from_list(
            "observed_value", rbtp_observed_values
        )
        self._rbtp.set_parameter_attributes_from_list(
            "observation_weight", rbtp_observation_weights
        )
    ...
    @property
    def rbtp(self) -> RigidBodyParameters:
    ...
    def estimate_parameters(self) -> np.array:
        ...
        for parameter in fields(self._rbtp):

            initial_value = getattr(self._rbtp, parameter.name).initial_value
            observed_value = getattr(self._rbtp, parameter.name).observed_value
            observation_weight = getattr(self._rbtp, parameter.name).observation_weight
    ...
    def estimate_parameter_uncertainties(self):
        ...
        for param_name in self._optim_results.params:
            if self._optim_results.params[param_name].vary:
                getattr(self._rbtp, param_name).estimated_uncertainty = np.sqrt(
                    Cxx[idx_prm, idx_prm]
                )
                idx_prm += 1
    ...
    def __residuals(
        ...
        weighted_rbtp_observation_residuals = (
            SimpleICPOptimization.__compute_weighted_rbtp_observation_residuals(params)
        )

        # Combine all residuals
        weighted_residuals = np.concatenate(
            (weighted_distance_residuals, weighted_rbtp_observation_residuals)
        )
    ...
    @staticmethod
    def __compute_weighted_rbtp_observation_residuals(
        params: lmfit.Parameters,
    ) -> np.array:
        """Compute weighted residuals for direct observation of rbtp."""
...
class RigidBodyParameters:
    ...
    def set_parameter_attributes_from_list(self, attribute_name: str, array: List):
        """Set an attribute for all six rbtp parameters from a given array.
    ...
    def get_parameter_attributes_as_list(self, attribute_name: str) -> List:
        """Returns an attribute of all six rbtp parameters as array.
